...
ant::analysis::Omega3::Omega3(const string &name, bool mctrue):
    Physics(name),
    run_on_true(mctrue)
{
...
void ant::analysis::Omega3::ProcessEvent(const ant::Event &event)
{
...
void ant::analysis::Omega3::Finish()
{

}

void ant::analysis::Omega3::ShowResult()
{
    canvas("Omega2")
...
double ant::analysis::Omega3::calcEnergySum(const ParticleList &particles) const
{
    double esum = 0.0;
...
ant::ParticleList ant::analysis::Omega3::getGeoAccepted(const ant::ParticleList &p) const
{
    ParticleList list;
    for( auto& particle : p) {
        if( geo.DetectorFromAngles(particle->Theta(), particle->Phi()) != detector_t::None )
